...
def main(idx,fileName,nrows=None,skip=0):
    # Convert train
    train = pd.read_csv(fileName, dtype='object', nrows=nrows,skiprows=skip, encoding='utf-8')
    train = develop_json_fields(df=train)
    logger.info('Train done')

    # Convert test
    # test = pd.read_csv('../data/kg-google/test_v2.csv', dtype='object', nrows=nrows, encoding='utf-8')
    ...
    # feature_to_drop = []
    feature_to_drop = ['customDimensions', 'hits' ]
    for f in train.columns:
        if f not in ['date', 'fullVisitorId', 'sessionId', 'totals.transactionRevenue']:
            if train[f].dtype == 'object':
                ...
                if (np.std(train[f].fillna(0).values) == 0):
                    # | (np.std(test[f].fillna(0).values) == 0):
                    feature_to_drop.append(f)
                    logger.info('No variation in {}'.format(f))
    ...
    if (idx ==0):
        # train.to_csv('../data/kg-google/' + 'extracted_fields_train.csv', index=False)
        train.to_csv('../data/kg-google/' + 'extracted_fields_test.csv', index=False)
    else:
        # train.to_csv('../data/kg-google/'+'extracted_fields_train.csv', index=False,mode='a', header=False)
        train.to_csv('../data/kg-google/' + 'extracted_fields_test.csv', index=False)
...
if __name__ == '__main__':
    ...
    try:
        ...
        loop = total //17
        # for i in range(0 , 16):
        #     main(i,'../data/kg-google/' + 'train_'+str(i)+'.csv')
        main(0, '../data/kg-google/' + 'test_v2.csv')
